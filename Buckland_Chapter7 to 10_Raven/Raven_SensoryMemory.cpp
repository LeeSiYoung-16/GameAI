...
void Raven_SensoryMemory::UpdateWithTargetDamage(HitTargetData* pTarget)
{
    ...
    if (m_pOwner != targetBot)
    {
        int damage = pTarget->damage;
        MakeNewRecordIfNotAlreadyPresent(targetBot);

        MemoryRecord& info = m_MemoryMap[targetBot];
        info.iLastDamageByEnemy = damage;

        if (m_pOwner->GetWorld()->isLOSOkay(m_pOwner->Pos(), targetBot->Pos()))
        {
            info.bShootable = true;
            info.vLastSensedPosition = targetBot->Pos();
        }
        else
        {
...
int Raven_SensoryMemory::FindTargetDamage(Raven_Bot* target)
{
    MemoryMap::const_iterator it = m_MemoryMap.find(target);

    return it->second.iLastDamageByEnemy;
}
